...
import { fetchMonthlyInflation, getCumulativeInflation, calculateRealValue } from '../services/InflationService';
...
interface DashboardProps {
  ...
  startMonthIndex: number; // Configurable start month (e.g., 6 for July)
}

const COLORS = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899', '#6366f1'];

export const Dashboard: React.FC<DashboardProps> = ({ data, incomeData, year, initialSavings, startMonthIndex }) => {
  ...
  useEffect(() => {
    ...
    fetchMonthlyInflation(year).then(setInflationRates);
  }, [year]);
  ...
  const projectionData = useMemo(() => {
    ...
    let runningActual = initialSavings;
    let runningProjected = initialSavings;
    ...
    return result;
  }, [monthlyData, initialSavings, avgSavings, currentMonthIndex, isReliableForStats, year, isCurrentYear, startMonthIndex, inflationRates]);
