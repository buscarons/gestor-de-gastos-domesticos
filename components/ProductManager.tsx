...
import { fetchMonthlyInflation, calculatePresentValue } from '../services/InflationService';
...
export const ProductManager: React.FC<ProductManagerProps> = ({ products, tags, onUpdateProducts, onUpdateTags, allExpenses }) => {
  ...
  useEffect(() => {
    fetchMonthlyInflation(new Date().getFullYear()).then(setInflationRates);
  }, []);
  ...
  const handleSaveProduct = () => {
    ...
    const finalTagId = tagId || (tags.length > 0 ? tags[0].id : 'default');
          ...
          {!isAdding && !editingId ? (
            ...
            <div className="space-y-5 animate-fade-in">
              ...
              <div className="pt-4 flex gap-2">
                ...
                <button onClick={handleSaveProduct} className="flex-1 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm font-medium flex items-center justify-center gap-2">
                  <Save size={16} />
                  Guardar
                </button>
              </div>
